...
class AuthViewModel(private val repository: RewardsRepository = RewardsRepository()) : ViewModel() {
    ...
    fun signIn(email: String, password: String) = viewModelScope.launch {
        ...
        runCatching { repository.signIn(email, password) }
            .onSuccess { _state.value = AuthState(signedIn = true, email = repository.currentEmail().orEmpty()) }
    ...
    fun signUp(email: String, password: String, name: String) = viewModelScope.launch {
        ...
        runCatching { repository.signUp(email, password, name) }
            .onSuccess { _state.value = AuthState(error = "Check your email to confirm your account.") }
            .onFailure { _state.value = AuthState(error = "Unable to create the account. Please try again.") }
    }
    ...
    fun signOut() = viewModelScope.launch { repository.signOut(); _state.value = AuthState() }
}
